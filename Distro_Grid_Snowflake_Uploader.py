...
def upload_SAFEWAY_distro_grid_to_snowflake(df, schema, table_name, selected_option):
    try:
        ...
        st.write(" they oh no match", selected_option)
        ...
        description = f"Started {selected_option} Start Achive Process for distro_grid table"

        insert_log_entry(user_id, "SQL Activity", description, True, local_ip, selected_option)
        description = ''

            # Step 1: Fetch data for archiving
        cursor.execute("SELECT * FROM DISTRO_GRID_TEST WHERE STORE_NAME = %s", (selected_option,))
        ...
        if data_to_archive:
            ...
            for row in data_to_archive:
                cursor.execute("INSERT INTO DISTRO_GRID_ARCHIVE (STORE_NAME, STORE_NUMBER, UPC, SKU, PRODUCT_NAME, MANUFACTURER, SEGMENT, YES_NO, ACTIVATION_STATUS, COUNTY, CHAIN_NAME, ARCHIVE_DATE) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (*row, current_date))

        # Log the start of the SQL activity
        #description = "Completed the archive process for the distro_grid table"
        description = f"Started {selected_option} Completed Achive Process for distro_grid table"
...
def upload_FOODMAXX_distro_grid_to_snowflake(df, schema, table_name, selected_option):
    ...
    try:
        ...
        if data_to_archive:
            ...
            # Step 3: Remove archived records from distro_grid_test table
